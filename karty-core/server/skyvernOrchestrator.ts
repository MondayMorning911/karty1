...
const SKYVERN_ENABLED = process.env.SKYVERN_ENABLED === 'true';
    ...
    for (let i = 0; i < photosUrl.length; i++) {
        ...
        try {
            ...
            if (url.startsWith('/') && fs.existsSync(url)) {
                ...
                ext = path.extname(url).replace('.', '') || 'jpg';
...
async function runSkyvernTask(task: SkyvernTask) {
    if (!SKYVERN_ENABLED || !process.env.SKYVERN_API_KEY) {
        throw new Error('Skyvern fallback is disabled or not configured');
    }
...
export async function executeSinglePortalFallback(
    userId: string,
    objectId: string,
    payload: KartyPayload,
    portal: KartyPayload['portal_targets'][number]
) {
    if (!SKYVERN_ENABLED || !process.env.SKYVERN_API_KEY) {
        return { success: false, error: 'Skyvern fallback is disabled or not configured' };
    }

    const portalDbKey = portal.replace('.ge', '').replace('.com', '');
    const profileEnvKey = `SKYVERN_PROFILE_ID_${portalDbKey.toUpperCase()}`;
    const browserProfileId = process.env[profileEnvKey];
    if (!browserProfileId) {
        return { success: false, error: `Skyvern browser profile is not configured: ${profileEnvKey}` };
    }

    const browserApiUrl = SKYVERN_API_URL.replace(/\/api\/v1\/?$/, '/v1');
    const sessionResponse = await fetch(`${browserApiUrl}/browser_sessions`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'x-api-key': process.env.SKYVERN_API_KEY },
        body: JSON.stringify({ timeout: 15, browser_profile_id: browserProfileId }),
    });
    if (!sessionResponse.ok) return { success: false, error: `Skyvern browser session failed: ${sessionResponse.status}` };
    const session = await sessionResponse.json() as any;
    const browserSessionId = session.browser_session_id;
    if (!browserSessionId) return { success: false, error: 'Skyvern returned no browser session ID' };

    try {
        const localPhotos = await downloadPhotosLocally(payload.photos, objectId, portal);
        if (localPhotos.length !== payload.photos.length) {
            return { success: false, error: 'Skyvern photo preparation failed' };
        }
        const task: SkyvernTask = {
            portal,
            url: getPortalUploadUrl(portal),
            navigation_goal: getSkyvernGoal(portal),
            data_extraction_goal: 'Extract only the public URL of the newly published listing after successful submission.',
            navigation_payload: { ...payload, local_photos: localPhotos },
            error_code_mapping: ERROR_CODE_MAPPING,
            max_steps_override: 50,
            browser_session_id: browserSessionId,
        };
        const result = await runSkyvernTask(task) as any;
        const listingUrl = String(result?.listing_url || '');
        const allowedHost = new URL(getPortalUploadUrl(portal)).hostname;
        if (!result?.success || !listingUrl || new URL(listingUrl).hostname !== allowedHost) {
            return { success: false, error: result?.error || 'Skyvern returned no verified listing URL' };
        }
        return { success: true, listing_url: listingUrl };
    } finally {
        cleanupLocalPhotos(objectId, portal);
        try {
            await fetch(`${browserApiUrl}/browser_sessions/${browserSessionId}/close`, {
                method: 'POST',
...
export async function executePortalPublishing(userId: string, objectId: string, payload: KartyPayload) {
    ...
    const tasksFuncs = payload.portal_targets.map(portal => async () => {
        ...
        try {
            ...
            // 2. Скачиваем фото локально (Self-hosted)
            const localPhotos = await downloadPhotosLocally(payload.photos, objectId, portal);
