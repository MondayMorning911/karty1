...
import { syncAuthStateToPython } from './authSync.js';
...
export class AuthManager {
  static async loginWithPassword(userId: string, platform: 'ssge' | 'myhome' | 'realting', loginStr: string, passwordStr: string) {
    ...
    try {
      if (process.env.AUTH_USE_BROWSERLESS === 'false') {
        throw new Error('Browserless disabled by AUTH_USE_BROWSERLESS');
      }
      console.log(`[AuthManager] Creating self-hosted Browserless session for ${platform}...`);
      browser = await chromium.connectOverCDP(wsUrl, { timeout: 45000 });
    } catch (remoteError: any) {
      if (process.env.AUTH_LOCAL_FALLBACK === 'false') throw remoteError;
      console.warn(`[AuthManager] Browserless unavailable, using local Chromium fallback: ${remoteError.message}`);
      usingLocal = true;
      const localExecutable = '/root/.cache/ms-playwright/chromium-1223/chrome-linux64/chrome';
      browser = await chromium.launch({
        headless: process.env.AUTH_LOCAL_HEADLESS !== 'false',
        executablePath: fs.existsSync(localExecutable) ? localExecutable : undefined,
    ...
    try {
      ...
      if (!usingLocal) await page.route('**/*', (route) => {
      ...
      const loginSelector = platform === 'ssge'
        ? 'input[name="userName"]'
        : platform === 'myhome'
          ? '#_r_m_'
          : '#loginform-username';
      ...
      if (platform === 'ssge') {
        await page.goto('https://home.ss.ge/ru/%D0%BD%D0%B5%D0%B4%D0%B2%D0%B8%D0%B6%D0%B8%D0%BC%D0%BE%D1%81%D1%82%D1%8C', { waitUntil: 'domcontentloaded', timeout: 60000 }).catch(e => console.warn('ss.ge entry goto warning:', e.message));
        const authLink = page.locator('a:has-text("Авторизация"), button:has-text("Авторизация"), a:has-text("Войти"), button:has-text("Войти")').first();
        if (await authLink.count() > 0) {
          await authLink.click({ timeout: 15000 });
          await page.waitForTimeout(1500);
        }
        loginFormReady = await page.locator(loginSelector).count() > 0;
      } else {
        for (let attempt = 0; attempt < 3 && !loginFormReady; attempt++) {
          await page.goto(targetUrl, { waitUntil: 'domcontentloaded', timeout: 60000 }).catch(e => console.warn(`goto warning (attempt ${attempt + 1}):`, e.message));
          loginFormReady = await page.locator(loginSelector).count() > 0;
          if (!loginFormReady) await delay(2000);
        }
      }
      ...
      // Do not report success when the provider left us on its login form.
      ...
      const loginPageStillOpen = platform === 'ssge'
        ? currentUrl.includes('account.ss.ge') && passwordFieldVisible
        : platform === 'myhome'
          ? currentUrl.includes('auth.tnet.ge') && passwordFieldVisible
          : false;
      const hasLoginError = /неверн|არასწორი|invalid password|incorrect password|wrong password|ошибка входа/.test(pageText);
      if (loginPageStillOpen || hasLoginError) {
        throw new Error('Площадка не приняла логин или пароль');
      }
      ...
      await syncAuthStateToPython(userId, platform, pwStorageState);
