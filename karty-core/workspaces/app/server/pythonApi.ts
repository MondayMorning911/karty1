...
export function startPythonApi() {
  ...
  pythonProcess = spawn('bash', ['-c', 'cd /workspaces/app/karty-lab-code && source venv/bin/activate && xvfb-run --auto-servernum --server-args="-screen 0 1280x900x24" python3 run_api.py'], {
    stdio: 'inherit',
    env: { ...process.env }
  });
