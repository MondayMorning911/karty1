...
class SsGeSite(BaseSite):
        ...
        if floor and prop_type not in ("land", "house"):
            try:
                ...
                if await floor_input.count() == 0:
                    ...
                    floor_input = self.page.evaluate("""() => {
                        const inputs = document.querySelectorAll('input');
                        for (const el of inputs) {
                            const name = el.name || '';
                            if (name.includes('floor') && !name.includes('floors')) return true;
                        }
                        return false;
                    }""")
    ...
    async def _publish(self) -> str:
        ...
        # Scroll to bottom and click "Продолжить →" button
            ...
            if btn_count > 0:
                try:
                    ...
                    if bbox and bbox['width'] > 30 and bbox['height'] > 20:
                        ...
                        clicked = True
                        self.log.info(f"Clicked '{btn_text}' via Playwright")
                        break
                except Exception as e:
        ...
        # Step 2: "Выберите статус" page — click "Размещение заявки"
                ...
                for key in ["url", "link", "id", "slug", "postId", "listingId", "applicationId"]:
                    if key in data:
                        ...
                        if isinstance(val, (int, str)):
                            return f"https://home.ss.ge/ru/l/{val}"
            except Exception:
                pass
        ...
        listing_url = await self.page.evaluate("""() => {
            ...
            for (const link of links) {
                ...
                if (href.includes('/l/') && /\\d{4,}/.test(href) && !href.includes('create') && !href.includes('?')) {
                    return href;
                }
            }
            return null;
    ...
    async def _delete_listing(self, url: str) -> bool:
        try:
            ...
            # Step 1: Find and click the trash icon (small button, right side of listing)
            clicked_trash = await self.page.evaluate("""() => {
                // Look for trash/delete icon buttons (small, on the right side)
                const btns = document.querySelectorAll('button');
                for (const btn of btns) {
                    const r = btn.getBoundingClientRect();
                    const text = btn.textContent.trim();
                    const ariaLabel = btn.getAttribute('aria-label') || '';
                    // Trash icons are typically small buttons
                    if (r.width > 0 && r.height > 0 && r.width < 40 && r.height < 40 &&
                        (text.includes('Удалить') || text.includes('Delete') || ariaLabel.includes('удалить') || ariaLabel.includes('delete'))) {
                        btn.scrollIntoView({block: 'center'});
                        btn.click();
                        return 'trash_icon';
                    }
                }
                // Fallback: find by SVG trash icon class or any small button with trash
                const icons = document.querySelectorAll('svg, [class*="trash"], [class*="delete"]');
            ...
            if clicked_trash == 'not_found':
                self.log.warning("Could not find trash icon")
            ...
            deleted = await self.page.evaluate("""() => {
                ...
                for (const btn of btns) {
                    const text = btn.textContent.trim();
                    const r = btn.getBoundingClientRect();
                    // The red "Удалить" button in modal is typically larger and colored
                    if (text === 'Удалить' && r.width > 80 && r.height > 30 && r.y > 0) {
                        btn.scrollIntoView({block: 'center'});
                        btn.click();
                        return 'clicked';
                    }
