...
export const PlatformLoginAuth = ({ onBack, siteKey, userId }: { onBack: () => void, siteKey: string, userId: string }) => {
  ...
  const handleNext = async () => {
    ...
    try {
      ...
      if (!session?.access_token) throw new Error('Сессия пользователя истекла. Откройте Mini App заново.');
      ...
      if (data.status === 'success') {
        ...
        setError(data.message || data.error || 'Не удалось выполнить авторизацию');
      }
    } catch (err: any) {
