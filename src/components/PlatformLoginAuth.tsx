...
export const PlatformLoginAuth = ({ onBack, siteKey, userId }: { onBack: () => void, siteKey: string, userId: string }) => {
  ...
  const getTheme = () => {
    switch (siteKey) {
      ...
      case 'realting':
        return {
          primaryStr: '#5BC8C0',
          btnClass: 'bg-[#E8643A] hover:bg-[#CF5A34] text-white shadow-[0_5px_15px_-3px_rgba(232,100,58,0.3)] tracking-wide',
          focusClass: 'focus:border-[#5BC8C0] focus:ring-1 focus:ring-[#5BC8C0]/30 dark:focus:border-[#5BC8C0]',
          headerClass: 'bg-[#5BC8C0] dark:bg-[#5BC8C0]/20 border-b border-[#4AB3AA]',
          headerTextClass: 'text-white dark:text-white font-semibold',
          backBtnClass: 'bg-white/20 text-white hover:bg-white/30',
          inputClass: 'bg-[#FFFFFF] dark:bg-black/50 border-slate-200 rounded-lg'
        };
      case 'myhome':
        return {
          primaryStr: '#4CAF7D',
          btnClass: 'bg-[#4CAF7D] border border-[#4CAF7D] hover:bg-[#3D8C64] text-white shadow-sm',
          focusClass: 'focus:border-[#4CAF7D] focus:ring-1 focus:ring-[#4CAF7D]/30 dark:focus:border-[#4CAF7D]',
          headerClass: 'bg-[#F5F5F5]/90 dark:bg-[#0A0A0A]/80 border-b border-slate-200 dark:border-white/5',
          headerTextClass: 'text-[#333333] dark:text-white font-medium',
          backBtnClass: 'bg-white dark:bg-white/10 text-[#4A90D9] shadow-sm hover:bg-slate-50 dark:hover:bg-white/20',
          inputClass: 'bg-[#F5F5F5] dark:bg-[#1A1A1A] border-transparent rounded-xl'
        };
      default:
        return {
          primaryStr: '#533afd',
  ...
  return (
    <div className="flex flex-col h-full bg-[#fcfdfd] dark:bg-[#0A0A0A] transition-colors duration-500 relative overflow-hidden text-[#061b31] dark:text-white">
      ...
      <div className={`flex items-center px-4 pt-4 pb-4 backdrop-blur-xl z-10 transition-colors ${theme.headerClass}`}>
        <button onClick={onBack} className={`w-8 h-8 flex items-center justify-center rounded-full mr-3 transition-colors ${theme.backBtnClass}`}>
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M15 18l-6-6 6-6"/></svg>
        </button>
        <div>
          <h1 className={`text-[20px] tracking-tight leading-tight ${theme.headerTextClass}`}>Вход в {getSiteName()}</h1>
        </div>
      </div>
      ...
      <div className="flex-1 p-6 z-10 overflow-y-auto">
          ...
          {step !== 'done' ? (
            <>
              ...
              <>
                ...
                <div>
                  <label className="block text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400 mb-1.5">Пароль</label>
                  <input 
                    type="password" 
                    value={password}
              ...
              <div className="mb-5 p-3 rounded-xl bg-orange-50 dark:bg-orange-500/10 border border-orange-100 dark:border-orange-500/20 text-orange-800 dark:text-orange-300 text-xs flex items-start gap-2">
                ...
                <p>Мы <b>не храним ваш пароль</b>. Он используется только один раз, сейчас, для создания зашифрованной сессии.</p>
              </div>
            ...
            <div className="text-center py-6">
              ...
              <p className="text-slate-500 dark:text-slate-400 text-sm mt-2 mb-6">Связь с {getSiteName()} установлена, вы можете публиковать объекты.</p>
              
              <button onClick={onBack} className="w-full py-3.5 bg-slate-100 dark:bg-white/10 hover:bg-slate-200 dark:hover:bg-white/20 rounded-xl font-medium transition-colors">
        ...
        <p className="text-center text-xs text-slate-500 mt-6">
          Karty безопасно шифрует данные и использует их напрямую для связи с {getSiteName()}.
        </p>
      </div>
    </div>
  );
