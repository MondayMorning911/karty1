...
class MyhomeGeSite(BaseSite):
    ...
    async def _check_balance(self) -> str:
        try:
            ...
            if not match:
                self.log.warning("MyHome balance was not visible; continuing to publish")
                return ""
            amount = float(match.group(1).replace(" ", "").replace(",", "."))
            self.log.info(f"MyHome balance precheck: {amount:.2f} GEL")
            if amount < 0.10:
                return f"MyHome balance is insufficient: {amount:.2f} GEL, required at least 0.10 GEL"
        except Exception as exc:
            self.log.warning(f"MyHome balance precheck failed; continuing to publish: {exc}")
        return ""
    ...
    async def _select_subtype(self, type_: str, subtype: str = ""):
        if type_ == "house" and subtype:
            ...
            try:
                room_type = self.page.get_by_text("Тип помещения", exact=False).first
                await room_type.click(timeout=5000)
                await asyncio.sleep(1)
                await self.page.get_by_text(subtype, exact=True).first.click(timeout=5000)
    ...
    async def _fill_fields(self, listing: dict):
        ...
        contact_phone = str(listing.get("contact_phone", "")).strip()
        if contact_phone:
            try:
                phone_input = self.page.locator("input[placeholder*='номер'], input[placeholder*='телефон']")
                if await phone_input.count() > 0:
                    await phone_input.first.scroll_into_view_if_needed()
                    await phone_input.first.fill(contact_phone, timeout=5000)
                    self.log.info("Set contact phone")
                else:
                    self.log.warning("Contact phone input not found")
            except Exception as e:
                self.log.warning(f"Failed to set contact phone: {e}")
        ...
        if "checkout" in new_url:
            ...
            try:
                ...
                if await balance_btn.count() > 0:
                    ...
                    raise RuntimeError("MyHome balance payment option is unavailable")
                # Click pay button
                ...
                if await pay_btn.count() > 0:
                    ...
                    if "/status/" not in new_url:
                        raise RuntimeError(f"MyHome payment did not complete: {new_url}")
                else:
                    raise RuntimeError("MyHome payment confirmation button is unavailable")
            except Exception as e:
                self.log.error(f"Payment failed: {e}")
                try:
                    payment_text = (await self.page.locator("body").inner_text()).lower()
                except Exception:
                    payment_text = ""
                if any(marker in payment_text for marker in ("недостаточно", "баланс", "insufficient")):
                    raise RuntimeError("MyHome balance is insufficient during payment") from e
                raise RuntimeError(f"MyHome payment failed: {e}") from e

        if "/status/" in new_url:
            try:
                dashboard_url = "https://statements.myhome.ge/ru/user-profile/my-statements?referrer=myhome"
                await self.page.goto(dashboard_url, wait_until="domcontentloaded", timeout=60000)
                await asyncio.sleep(8)
                dashboard_listing_url = await self.page.evaluate(r"""() => {
                    const links = document.querySelectorAll("a[href*='/pr/']");
                    for (const link of links) {
                        if (/\/pr\/\d+\//.test(link.href)) return link.href;
                    }
                    return null;
                }""")
                if dashboard_listing_url:
                    self.log.info(f"Found listing URL from dashboard: {dashboard_listing_url}")
                    return dashboard_listing_url
            except Exception as e:
                self.log.warning(f"Dashboard listing lookup failed: {e}")
            self.log.warning("Payment succeeded but no public listing URL was found")
            return ""
    ...
    async def _find_recent_listing_url(self, listing: dict) -> str:
        await self.page.goto("https://statements.myhome.ge/ru/user-profile/my-statements?referrer=myhome", wait_until="domcontentloaded", timeout=60000)
        await asyncio.sleep(8)
        ...
        if not address and not price:
            return ""
        return await self.page.evaluate(r"""({ address, price, listingType, listingDeal }) => {
            ...
            const cards = links.map(link => ({
                link,
                text: normalize((link.closest('article, li, [class*="card"], [class*="item"], div') || link.parentElement)?.textContent || link.textContent),
            }));
            const exact = cards.find(({ text }) => addressText.length >= 4 && text.includes(addressText) && priceText && text.includes(priceText));
            if (exact) return exact.link.href;
            const byAddress = cards.find(({ text }) => addressText.length >= 4 && text.includes(addressText));
            if (byAddress) return byAddress.link.href;
            // MyHome may display USD input prices converted to GEL in the cabinet.
            // The newest matching /pr/ card is safer than treating /status/success as a listing.
            const type = normalize(String(listingType || ''));
            const deal = normalize(String(listingDeal || ''));
            const typed = cards.find(({ text }) => {
                const typeMatch = !type || (type === 'land' ? text.includes('земел') || text.includes('участ') : type === 'commercial' ? text.includes('коммер') || text.includes('площад') : type === 'house' ? text.includes('дом') : text.includes('кварт'));
                const dealMatch = !deal || (deal === 'rent' ? text.includes('аренд') || text.includes('сда') : text.includes('прода'));
                return typeMatch && dealMatch;
            });
            if (typed) return typed.link.href;
            return links[0]?.href || '';
        }""", {"address": address, "price": price, "listingType": listing.get("type", ""), "listingDeal": listing.get("deal", "")})
    ...
    async def _delete_listing(self, url: str) -> bool:
        try:
            ...
            # Only delete the exact listing URL. Never fall back to the first card.
            del_info = await self.page.evaluate("""(targetUrl) => {
                const target = String(targetUrl || '').split('?')[0].replace(/\\/$/, '');
                const targetLink = [...document.querySelectorAll('a[href]')].find(link => link.href.split('?')[0].replace(/\\/$/, '') === target);
                if (!targetLink) return null;
                let card = targetLink;
                for (let level = 0; level < 8 && card; level++, card = card.parentElement) {
                    const elements = card.querySelectorAll('*');
                    for (const el of elements) {
                        const r = el.getBoundingClientRect();
                        if (r.width > 0 && r.height > 0 && r.height < 40 && r.width < 200 && el.textContent.trim() === 'Удалить') {
            ...
            if del_info:
                ...
                if da_info:
                    ...
                    still_in_cabinet = await self.page.evaluate("""(target) => [...document.querySelectorAll('a[href]')]
                        .some(link => link.href.split('?')[0].replace(/\\/$/, '') === target)""", target)
                    self.log.info(f"MyHome exact delete verification: cabinet_link_present={still_in_cabinet}")
                    return not still_in_cabinet
